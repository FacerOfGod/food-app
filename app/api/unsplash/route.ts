...
export async function GET(request: NextRequest) {
  ...
  if (!guard.q) return NextResponse.json({ photos: [] });
  ...
  const res = await fetch(
    `https://api.unsplash.com/search/photos?query=${encodeURIComponent(guard.q)}&per_page=6&orientation=landscape`,
